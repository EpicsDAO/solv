...
export const setupKeys = (commission = 10, isLocal = false) => {
  try {
    ...
    if (isLocal) {
      ...
      voteKey = `${SOLV_KEYPAIR_UPLOAD_PATH}/${VALIDATOR_VOTE_KEYFILE}`
      validatorTestnetKey = `${SOLV_KEYPAIR_UPLOAD_PATH}/${TESTNET_VALIDATOR_KEYFILE}`
      validatorMainnetKey = `${SOLV_KEYPAIR_UPLOAD_PATH}/${MAINNET_VALIDATOR_KEYFILE}`
    }
